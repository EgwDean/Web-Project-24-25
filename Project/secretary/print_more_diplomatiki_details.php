...
$stm = "SELECT id_diplomatiki AS ID, title AS Θέμα , description AS Περιγραφή, anathesi_diplomatikis.status AS Κατάσταση, email_prof AS Επιβλέπων, member1 AS Καθηγητής_Α, member2 AS Καθηγητής_Β, TIMESTAMPDIFF(DAY, start_date, NOW()) AS Ημέρες_από_Ανάθεση, protocol_number AS Αριθμός_Πρωτοκόλλου
FROM diplomatiki
...
WHERE (anathesi_diplomatikis.status = 'active' OR anathesi_diplomatikis.status = 'under examination' OR anathesi_diplomatikis.status = 'canceled_by_student') AND id_diplomatiki = ?";
